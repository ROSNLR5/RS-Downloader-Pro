  ...
  const checkGitHubUpdates = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const cleanCurrent = '1.2.9';
        
        if (cleanLatest !== cleanCurrent) {
          setUpdateCheckState('new-version');
          setCheckMessage(`¡Nueva versión detectada! Versión ${latestTag} disponible en GitHub (actual: v1.2.9).`);
        } else {
          setUpdateCheckState('latest');
          setCheckMessage(`¡Felicidades! La versión local v1.2.9 coincide con la última versión de producción en GitHub.`);
        }
      } else {
        // Custom friendly fallback simulation
        setUpdateCheckState('latest');
        setCheckMessage(`El repositorio github.com/${repoOwner}/${repoName} es privado o aún no tiene un Release público. (Simulación: v1.2.9 es la versión más reciente).`);
      }
    } catch (err) {
